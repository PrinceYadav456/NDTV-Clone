import React from "react";
import latestData from "../../data/latestdata.json";
import "./Cricket.css";

const Cricket = () => {
  const keywords = ["cricket", "ipl", "match", "t20", "odi", "test", "batsman", "bowler"]; 
  const filtered = latestData.filter((item) => {
    const text = (
      (item.title || "") + " " + (item.description || "") + " " + (item.source?.name || "") + " " + (item.content || "")
    ).toLowerCase();
    return keywords.some((k) => text.includes(k));
  });

  const items = filtered.length ? filtered : latestData.slice(0, 8);

  return (
    <div className="cricketPage">
      <h1>Cricket</h1>
      <div className="cricketList">
        {items.map((it, i) => (
          <a
            key={i}
            href={it.url}
            target="_blank"
            rel="noreferrer"
            className="cricketLink"
          >
            <div className="cricketRow">
              {it.urlToImage ? (
                <img src={it.urlToImage} alt="" className="cricketImage" />
              ) : null}
              <div>
                <h3 className="cricketTitle">{it.title}</h3>
                <p className="cricketDate">{it.publishedAt}</p>
              </div>
            </div>
          </a>
        ))}
      </div>
    </div>
  );
};

export default Cricket;
